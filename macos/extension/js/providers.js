(function (root) {
  ...
  var http = root.LaSubtituletaHttp;
  var transcript = root.LaSubtituletaTranscript;
  ...
  function transcribe(options, onProgress) {
    ...
    if (provider === "assemblyai") {
      request = assemblyAI(options, model, onProgress);
    } else if (provider === "speechmatics") {
      request = speechmatics(options, model, onProgress);
    } else if (provider === "deepgram") {
      request = deepgram(options, model, onProgress);
    } else {
  ...
  async function assemblyAI(options, model, onProgress) {
    ...
    var understandingRequest = {};
    if (options.identifySpeakers) {
      payload.speaker_labels = true;
      if (Number(options.speakerCount) > 1) {
        payload.speakers_expected = Number(options.speakerCount);
      }
      var speakerNames = transcript.parseSpeakerNames(options.speakerNames);
      if (speakerNames.length) {
        understandingRequest.speaker_identification = {
          speaker_type: "name",
          speakers: speakerNames.map(function (name) { return { name: name }; })
        };
      }
    }
    if (isTranslation(options)) {
      ...
      payload.speaker_labels = true;
      understandingRequest.translation = {
        target_languages: [options.outputLanguage],
        match_original_utterance: true,
        formal: false
      };
    }
    if (Object.keys(understandingRequest).length) {
      payload.speech_understanding = { request: understandingRequest };
    }
    ...
    if (isTranslation(options)) {
      ...
      words = assemblyWords(result, options.identifySpeakers);
    }
  ...
  function assemblyWords(result, useUtterances) {
    var source = [];
    if (result.utterances && result.utterances.length) {
      result.utterances.forEach(function (utterance, utteranceIndex) {
        var utteranceWords = utterance.words || [];
        utteranceWords.forEach(function (word, wordIndex) {
          source.push({
            text: word.text,
            start: word.start,
            end: word.end,
            confidence: word.confidence,
            speaker: word.speaker == null ? utterance.speaker : word.speaker,
            utteranceId: "assembly:" + utteranceIndex,
            boundaryBefore: wordIndex === 0,
            boundaryAfter: wordIndex === utteranceWords.length - 1
          });
        });
      });
    } else {
    ...
    return source.map(function (word) {
      return {
        text: word.text,
  ...
  function translatedAssemblyWords(result, language) {
    ...
    (result.utterances || []).forEach(function (utterance, utteranceIndex) {
      ...
      if (translated) {
      output = output.concat(transcript.distributeText(
        translated,
        utterance.start / 1000,
        utterance.end / 1000,
        utterance.confidence,
        utterance.speaker,
        { utteranceId: "assembly-translation:" + utteranceIndex, boundaryBefore: true, boundaryAfter: true }
      ));
      }
    });
  ...
  async function deepgram(options, model, onProgress) {
    ...
    var words = [];
    var utterances = data.results && data.results.utterances;
    if (utterances && utterances.length) {
      utterances.forEach(function (utterance, utteranceIndex) {
        var utteranceWords = utterance.words || [];
        utteranceWords.forEach(function (word, wordIndex) {
          words.push({
            ...
            confidence: word.confidence,
            speaker: word.speaker == null ? utterance.speaker : word.speaker,
            utteranceId: "deepgram:" + (utterance.id || utteranceIndex),
            boundaryBefore: wordIndex === 0,
            boundaryAfter: wordIndex === utteranceWords.length - 1
          });
        });
      ...
      words = (alternative.words || []).map(function (word) {
        return {
          text: word.punctuated_word || word.word,
          start: word.start,
          end: word.end,
    ...
    return normalizedResult(options, words, detected, {
      provider: "deepgram",
      requestId: data.metadata && data.metadata.request_id,
      audioDuration: Number(data.metadata && data.metadata.duration || 0)
    });
  }
  ...
  async function speechmatics(options, model, onProgress) {
    ...
    var transcriptionConfig = {
      ...
      diarization: options.identifySpeakers ? "speaker" : "none"
    };
    ...
    var result = await http.json("https://asr.api.speechmatics.com/v2/jobs/" + submitted.id + "/transcript?format=json-v2", {
      headers: { Authorization: "Bearer " + key }
    });
    ...
    return normalizedResult(options, words, language || options.inputLanguage, {
      ...
      jobId: submitted.id,
      audioDuration: Number(result.metadata && result.metadata.duration || 0)
    });
  }
  ...
  function speechmaticsWords(results) {
    ...
    (results || []).forEach(function (item) {
      ...
      output.push({
        ...
        speaker: alternative.speaker == null ? item.speaker : alternative.speaker,
        boundaryBefore: pendingBoundary,
        boundaryAfter: Boolean(item.is_eos),
        sentenceEnd: Boolean(item.is_eos)
      });
  ...
  function translatedSpeechmaticsWords(result, language) {
    ...
    segments.forEach(function (segment, index) {
      ...
      output = output.concat(transcript.distributeText(
        ...
        1,
        segment.speaker,
        { utteranceId: "speechmatics-translation:" + index, boundaryBefore: true, boundaryAfter: true }
      ));
    });
    return output;
  }

  function normalizedResult(options, words, detectedLanguage, providerMetadata) {
    var normalized = transcript.normalizeWords(words);
    var speakers = [];
    if (options.identifySpeakers) {
      speakers = transcript.createSpeakerMap(normalized, options.speakerNames);
      normalized = transcript.applySpeakerMap(normalized, speakers);
    }
    ...
    return {
      ...
      speakerNames: transcript.parseSpeakerNames(options.speakerNames),
      speakers: speakers,
  ...
  root.LaSubtituletaProviders = { transcribe: transcribe };
})(window);
